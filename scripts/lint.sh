...
sed -i 's/-forward-unknown-to-host-compiler/ /g' build/compile_commands.json
sed -i 's/-rdc=true/ /g' build/compile_commands.json
sed -i 's/--generate-code=arch=compute_[[:digit:]]*,code=\[compute_[[:digit:]]*,sm_[[:digit:]]*\]/ /g' build/compile_commands.json
