...
DTYPE* allocate1DArrayDevice(size_t n) {
    DTYPE* allocated;
    gpuCheckError(cudaMalloc(&allocated, n * sizeof(DTYPE))) return allocated;
}
...
void copy2DFromHostToDevice(DTYPE** host, DTYPE* device, size_t n, size_t m) {
    ...
    gpuCheckError(cudaMemcpy(device, temp, n * m * sizeof(DTYPE), cudaMemcpyHostToDevice)) free(temp);
}
...
DTYPE* copy1DArrayDevice(size_t n, DTYPE* old) {
    ...
    gpuCheckError(cudaMemcpy(allocated, old, n * sizeof(DTYPE), cudaMemcpyDeviceToDevice)) return allocated;
}
